...
public class EnemyGuardController : EnemyController
{
    ...
    public LayerMask flyingPossessedEnemyLayer;
    ...
    public void ChasePlayer()
    {
        ...
        if (DetectPlayer())
        {
            ...
            if (playerTarget.gameObject.layer == 18 || playerTarget.gameObject.layer == 11)
            {
                if (!isTouchingPlayer)
                {
                    if (playerTarget.position.x > transform.position.x)
    ...
    public bool DetectPlayer()
    {
        ...
        bool hitFlyingPossessedEnemy = DetectFlyingPossessedEnemy();
        ...
        if (hitPossessedEnemy || hitFlyingPossessedEnemy)
        {
            playerTarget = playerController.gameObject.transform;
        }

        if(hitPlayerHealth || hitPossessedEnemy || hitFlyingPossessedEnemy)
        {
    ...
    public bool DetectFlyingPossessedEnemy()
    {
        bool hitFlyingPossessedEnemy = false;
        Vector3 rayCastOrigin = new Vector3(raycastsStartPoint.position.x + (playerDetectorLength / 2), raycastsStartPoint.position.y, raycastsStartPoint.position.z);

        for (int i = 0; i < playerDetectorRaycasts; i++)
        {
            RaycastHit2D hit = Physics2D.Raycast(rayCastOrigin, Vector2.left, playerDetectorLength, flyingPossessedEnemyLayer);
            if (hit.collider)
            {
        ...
        return hitFlyingPossessedEnemy;
    }
