...
public class SSPlayerController : MonoBehaviour
{
    ...
    public bool spiritIsBeingDrained;
    ...
    public Color spiritDamageColor;
    ...
    public void StartTransitionToPlayer()
    {
        isTransitioning = true;
        ChangeStateToFrozen();
        SwitchToPlayerBeing();
    ...
    {
        if(currentSpiritHealth <= 0) { return; }
    ...
    public void DepossessEnemy()
    {
        ...
        if(currentlyPossessedEnemy != null)
        {
            EnemyController enemyController = currentlyPossessedEnemy.GetComponent<EnemyController>();
            EnemyHealth enemyHealth = currentlyPossessedEnemy.GetComponent<EnemyHealth>();
    ...
    private IEnumerator DecreaseSpiritHealth()
    {
        ...
        if (currentSpiritHealth <= 0 && isSpirit && !isTransitioning)
        {
            StartTransitionToPlayer();
            yield break;
        }
    ...
    public void DrainSpirit()
    {
        if (spiritIsBeingDrained)
        {
            return;
        }
        spiritIsBeingDrained = true;
        currentSpiritHealth -= 1;
        StartCoroutine(FlickerSprite());
        Invoke("StopSpiritDrain", 1f);
    }

    public IEnumerator FlickerSprite()
    {
        spriteRenderer.color = spiritDamageColor;
    ...
    public void StopSpiritDrain()
    {
        spiritIsBeingDrained = false;
    }
    ...
    public void UpdateSpiritHealth()
    {
    ...
    public void PlayOneShotSoundEffect(string action)
    {
